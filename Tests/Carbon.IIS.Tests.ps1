...
Describe ('Carbon.IIS.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.IIS' -Exists
    }
}

Describe ('Carbon.IIS.command verbs') {
...
Describe ('Carbon.IIS.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.IIS' -CommandType Function,Cmdlet,Filter))
        {
