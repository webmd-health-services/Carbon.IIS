...
{
    Uninstall-CIisAppPool -Name $SiteName
}

function Start-Test
{
    Uninstall-CIisWebsite -Name $SiteName
    Install-CIisWebsite -Name $SiteName -Path $TestDir -Bindings "http://*:$Port" -AppPoolName $SiteName
...
{
    Install-CIisVirtualDirectory -SiteName $SiteName -Name $VDirName -Path $Path
...
{
    Install-CIisVirtualDirectory -SiteName $SiteName -VirtualPath ('/{0}/' -f $VDirName) -PhysicalPath $PSScriptRoot
    Assert-VirtualDirectoryRunning
...
{
    Install-CIisVirtualDirectory -SiteName $SiteName -Name $VDirName -Path "$TestDir\..\..\Tests\IIS"
    $physicalPath = Get-CIisWebsite -SiteName $SiteName |
...
function Test-ShouldCreateDoubleVitualDirectory
{
    ...
    Install-CIisVirtualDirectory -SiteName $SiteName -VirtualPath $virtualPath -PhysicalPath $PSScriptRoot
...
{
    Invoke-NewVirtualDirectory
    Enable-CIisDirectoryBrowsing -SiteName $SiteName -Path $VDirName
...
{
    $output = Install-CIisVirtualDirectory -SiteName $SiteName -VirtualPath $VDirName -PhysicalPath $PSScriptRoot
    Assert-Null $output

    $app = Get-CIisApplication -SiteName $SiteName 
    ...
    $output = Install-CIisVirtualDirectory -SiteName $SiteName -VirtualPath $VDirName -PhysicalPath $PSScriptRoot -Force
    Assert-Null $output
