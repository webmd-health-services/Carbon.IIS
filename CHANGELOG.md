...
* No longer sets the default anonymous authentication username on a website to nothing. New websites will now have the
default anonymous authentication username of `IUSR`. To preserve the previous behavior, use the
...
* Configure all application pool settings (i.e. all settings stored on an application pool's `add` element in IIS'
applicationHost.config file. Added parameters `QueueLength`, `AutoStart`, `Enable32BitAppOnWin64`,
`ManagedRuntimeLoader`, `EnableConfigurationOverride`, `ManagedPipelineMode`, `CLRConfigFile`, `PassAnonymousToken`, and
`StartMode` to `Install-CIisAppPool`.
* Function `Set-CIisWebsite`, for setting a website's `id` and `serverAutoStart` properties.
* Parameter `ServerAutoStart` to `Install-CIisWebsite` that configures a website's `serverAutoStart` setting.
