...
<#
.SYNOPSIS
Imports the Carbon.IIS module into the current session.

.DESCRIPTION
The `Import-Carbon.IIS.ps1` script imports the Carbon.IIS module into the current session. If the module is already
loaded, it is removed, then reloaded.

.EXAMPLE
.\Import-Carbon.IIS.ps1

Demonstrates how to use this script to import the Carbon.IIS module  into the current PowerShell session.
#>
[CmdletBinding()]
param(
)
...
try
{
    ...
    Import-Module -Name (Join-Path -Path $PSScriptRoot -ChildPath 'Carbon.IIS.psd1' -Resolve)
}
finally
{
